...
function makeDoc(filename, options) {
    ...
    var baseTitle = options.title || (gitName || '').split('/').pop() + ' API Doc';
    ...
    var docFile = './doc/' + basename(filename, '.js') + '.html';

    console.log('writing file', docFile);
    files.push({name: docFile, className: constr ? constr.className : modexp});
    ...
    function appendCodeBlock(code) {
        ...
        body += '<pre class="prettyprint linenums:' + line + '" style="display: none; margin-top: 15px;"><code>' + code + '</code></pre>';
        body += '</div>';
        body += '<hr/>';
    }
