...
Parser.prototype.anyMethod = function () {
    console.log(this.filename, this.classMethods.length, this.instanceMethods.length, this.helperMethods.length);
    return (this.classMethods.length > 0) || (this.instanceMethods.length > 0) || (this.helperMethods.length > 0);
};
...
Parser.prototype.process = function process() {
    ...
    // this.fixMassPrototypeAssignment();
...
/**
 * Match __defineGetter__ declarations
 */
Parser.prototype.matchDefineGetter = function matchDefineGetter(m) {
    var reDefGetter = /^(.*?)\.__defineGetter__\(["'](.*?)["']/;
    var md = m.match[2].match(reDefGetter);
    if (!md) return false;
    this.helperMethods.push({
        methodName: md[2],
        declaration: m.match[2],
        doc: m.doc,
        isPublic: !!m.doc.match(/@(api )?public/),
        isHelperMethod: true,
        code: m.match[2] + m.code
    });
    return true;
};
...
Parser.prototype.fixMassPrototypeAssignment = function fixMassPrototypeAssignment() {
    var detectedMPA = false;
    var detectedBlock = false;
    var codeLines = this.codeLines;
    codeLines.forEach(function (line, i) {
        if (!detectedMPA) {
            var ctor = line.match(/^([A-Z].*?\.prototype)\s*=\s*\{\s*$/);
            if (ctor) {
                detectedMPA = ctor[1];
                console.log('mpa', detectedMPA);
                codeLines[i] = '';
            }
        } else {
            if (line.match(/^\}/)) {
            ...
            if (detectedBlock) {
                codeLines[i] = line.substr(detectedBlock.length);
                var eb = line.match(/^(\s+?)\}/);
                if (eb && eb[1].length === detectedBlock.length) {
                    detectedBlock = false;
                }
                return;
            }
            var ml = line.match(/^(\s*,?\s*)([\$_a-z].*?)\s*\:\s*(function.*)$/);
            if (ml) {
                detectedBlock = ml[1];
                codeLines[i] = detectedMPA + '.' + ml[2] + ' = ' + ml[3];
            }
        }
    });
};
