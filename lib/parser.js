    ...
    while (match = this.rule.exec(this.code)) {
        (function (match) {
            ...
            var m = {
                match: match,
                doc: doc,
                what: match[2].split(' = ')[0].split('.'),
                declaration: match[2].split(' = ')[1],
                code: match[3]
            };
...
/**
 * Try to match odd-style constructor looking like: `var Constructor = function () {`
 */
Parser.prototype.matchOddStyleConstructor = function matchOddStyleConstructor(m) {
    ...
    this.constr = {
        className: ctor,
        methodName: ctor,
        code: m.what[0] + m.declaration + code,
        doc: m.doc
    };
    return true;
};
